"""Quad16 at its native 50-sample resolution, adapted to a host's exact grid.

Bands are concatenated (not mean-pooled); one linear map changes feature
...
class QuadHostTokens(nn.Module):
...
class QuadCBraEmbedding(nn.Module):
    def __init__(self, native):
        super().__init__()
        self.positional_encoding=native.positional_encoding
        self.quad=QuadHostTokens(200,200)
...
class QuadBIOTEncoder(nn.Module):
    def __init__(self,native):
        super().__init__()
        self.native=native
        self.quad=QuadHostTokens(256,native.hop_length,native.n_fft)
...
def build_quad_host(cfg,input_shape):
    from ..build import build_model
    native_cfg=dict(cfg)
    host=cfg['model'].removesuffix('_quad16')
    ...
    with torch.random.fork_rng(devices=[]):
        if host=='cbramod':
            native.backbone.patch_embedding=QuadCBraEmbedding(native.backbone.patch_embedding)
        elif host=='biot':
            native.biot=QuadBIOTEncoder(native.biot)
