...
## Retire Anchor16 and Residual16 by explicit user instruction
User rejects spending further CHBMIT compute on architectures whose TUEV is weak. Terminate only CHB trainer63799/64400 with cooperative TERM,180s fallback KILL; preserve existing Quad16 TUEVseed1/2 (both completed15epochs at stop initiation). Both old lifecycle controllers and their rescue processes paused before retirement to prevent them releasing the allocation and killing Quad16. New srun supervisor scripts/retire_old_419232.py restores controllers after Quad16 finishes; receipt results/retire-old-419232.json.
Current allocation has only~4h left; extension to48h was rejected by Slurm permission checks. No new CHBMIT confirmation started into a known-insufficient window. User-authorized CHBMITseed1/2 remain assigned145GPU2/3; source transfer complete and destination transfer in progress, auto smoke/train after full checksum match. No further Anchor/Residual runs or confirmations. Old partial CHB results are operator-stopped, not completed failures.

## ADFD small-dataset confirmations in remaining allocation
User explicitly requested small-dataset seed confirmations to fill idle cards. ADFDseed1/2 on419232 HIP1/3, srun launcher PIDs1888193/1888194. Original seed0 config and30epochs retained; seed changed, administrative max_hours reduced8->3 to leave deadline margin. Seed0 completed~1h on145, so admit with GPU smoke and3h cap; a budget stop remains censored. Both TUEV confirmations HIP0/2 preserved. Four seeds now admitted in same existing node, no new allocation. Replacement lifecycle guardian scripts/hold_quad16_419232.py monitors all Quad16 training/smoke/wrapper processes, replacing narrow TUEV-only guardian74929, and releases original controllers only after every confirmation exits. ADFDseed0 used145 CUDA/torch2.8; seed1/2 useAMD ROCm/torch2.7.1: hardware/software differ, so these are seed confirmations across environments, not a pure hardware-controlled variance experiment.

## Completed TUEV three-seed confirmation
All3 completed20epochs, test kappa seed0/1/2 =0.727893403981,0.695187254471,0.656620445153. Mean0.693233701202, sampleSD0.035676616191. A128 matched3seeds mean0.689548555995,SD0.035665042364; PAC8/rot2 mean0.732774345368,SD0.016094180895. Therefore Quad16 preserves A128-level TUEV performance but does not establish stable~0.73 performance or surpass PAC8. Do not continue reporting seed0 as representative. ADFDseed2 completed bypatience27epochs,testbalanced0.461488100717; seed1 stillrunning atfirstcheck.145CHBMIT transfer30/43GB, seedsnotlaunched; b2TUSZ remainsPriority. Pretraining/host replacement have not been launched.
