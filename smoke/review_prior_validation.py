"""Fixed tau=1 train-prior adjustment diagnostic on validation only.
...
from paclock_bench.training.metrics import compute_metrics
...
def main():
    ...
    dest = Path('results') / ('prior-validation-' + job + '.json')
    ...
    for spec in args.runs:
        ...
        counts = np.asarray(result['class_counts']['train'], dtype=np.float64)
        assert counts.shape == (classes,) and (counts > 0).all() and np.isfinite(counts).all()
        log_prior = np.log(counts / counts.sum())
        adjusted = logits.astype(np.float64) - log_prior[None, :]
        # One prespecified setting, no validation fitting or parameter sweep.
        adjusted_metrics = compute_metrics(labels, adjusted, classes)
        acm = np.bincount(labels.astype(int) * classes + adjusted.argmax(1),
                         minlength=classes * classes).reshape(classes, classes)
        row.update(training_class_counts=counts.astype(int).tolist(), tau=1.0,
                   adjusted_metrics=adjusted_metrics,
                   adjusted_confusion_matrix=acm.tolist(),
                   adjusted_class_recall=(acm.diagonal()/acm.sum(1)).tolist(),
                   adjustment_note='Post-hoc logits minus log(training prior). Validation-only diagnostic, not a new trained candidate.')
        rows.append(row)
    ...
    print('PRIOR_VALIDATION_COMPLETE', dest, flush=True)
