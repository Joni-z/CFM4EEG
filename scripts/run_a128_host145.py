...
ROOT = pathlib.Path('/data2/zz5070/CFM4EEG-a128-r1-20260916')
...
assert ds in ['chbmit'] and seed in [0, 1, 2]
os.chdir(ROOT)
cfgpath = ROOT / f'configs/a128_hosts/{ds}_cbramod_s{seed}.yaml'
...
locks = pathlib.Path('/data2/zz5070/CFM4EEG-quad16-breadth-20260915/results/shared-gpu-locks')
locks.mkdir(exist_ok=True)
lock = open(locks / f'run-a128host-{ds}-s{seed}.lock', 'a')
try:
...
try:
    ...
    commands = [
        ['timeout', '--kill-after=60s', '30m', PY, '-u', 'smoke/verify_a128_hosts.py', '--config', str(cfgpath)],
    ...
    for i, cmd in enumerate(commands):
        save(phase='training' if i == 1 else 'smoking', stage=i)
